...
$(document).ready(function () {
    ...
    $(document).on('click', '.Product__Page__Content__Left__Filter__Cate__Item', function() {
        $('.Product__Page__Content__Left__Filter__Cate__Item').map(function() {
            $(this).removeClass('active')
        })
        $(this).addClass('active')
        $('#FilterBrand').val($(this).attr('value'))
    })

    $(document).on('click', '.Product__Page__Content__Left__Filter__Ram__Item', function() {
        $('.Product__Page__Content__Left__Filter__Ram__Item').map(function() {
            $(this).removeClass('active')
        })
        $(this).addClass('active')
        $('#FilterRam').val($(this).attr('value'))
    })

    $(document).on('click', '.Product__Page__Content__Left__Filter__Rom__Item', function() {
        $('.Product__Page__Content__Left__Filter__Rom__Item').map(function() {
            $(this).removeClass('active')
        })
        $(this).addClass('active')
        $('#FilterRom').val($(this).attr('value'))
    })

    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    var getBrand = urlParams.get('ProductBrand')
    var getRam = urlParams.get('ProductRam')
    var getRom = urlParams.get('ProductRom')
    $('.Product__Page__Content__Left__Filter__Cate__Item').map(function() {
    ...
    $('.Product__Page__Content__Left__Filter__Ram__Item').map(function() {
        if ($(this).attr('value') == getRam) {
            $(this).addClass('active')
        }
    })
    $('.Product__Page__Content__Left__Filter__Rom__Item').map(function() {
        if ($(this).attr('value') == getRom) {
            $(this).addClass('active')
        }
    })
}) 